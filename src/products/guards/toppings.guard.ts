// the guard will stop loading to many time also because we are checking if the toppings were loaded already
...
export class ToppingsGuard implements CanActivate {
  ...
  checkStore(): Observable<boolean> {
    return this.store.select(fromStore.getToppingsLoaded).pipe(
      // toppingsloaded returns an observable boolean
      tap(loaded => {
        if (!loaded) {
          this.store.dispatch(new fromStore.LoadToppings()); // tap does not touch the observable
